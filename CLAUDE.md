...
- **Don't hand-roll what a library does.** When a well-maintained crate exists for a standard, use it. JS identifier validity: `ID_Start (ID_Continue)*` via Unicode categories — use `unicode_ident::is_xid_start`/`is_xid_continue` (plus `$`) from the `unicode-ident` crate, NOT `[a-zA-Z_$][a-zA-Z0-9_$]*`. JS string escaping: use `serde_json::to_string`, not `format!("\"{}\"", s)`.
- **Correctness is non-negotiable — 100%, always.** If a standard says X, implement X. "Overkill" only applies with positive proof the edge case cannot occur. Without that proof, implement it correctly. Never defend a shortcut with "our inputs are ASCII-only" or "this won't come up in practice".
...
- **Never silently guess at semantics.** When implementing behavior that depends on external definitions (format specs, bytecode encodings, type mappings, runtime semantics), find an authoritative source first. If you can't, record the assumption in TODO.md for verification. An unverified guess that isn't written down is indistinguishable from a verified fact to the next session.
...
- **Stubs must throw, not silently fail.** Implement the function, or throw `Error("name: not yet implemented")` and add a TODO.md entry immediately. Silent returns (0, `""`, `false`, `null`, empty `{}`) are worse than throws — they let code run with missing functionality and fail silently far from the source. Every throw-stub requires a TODO.md entry at the time it's written, no exceptions. For platform APIs with no browser equivalent, use an explicit no-op with a comment: `/* no-op — PSN commerce not available in browser */` — a silent `return false` is wrong in all cases.
