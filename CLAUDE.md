...
- **Never widen a runtime API to suppress a TS2345 / TS2554.** When emitted code passes a value of the wrong type to a runtime function, the diagnostic is almost always correct — it reflects a gap in the pipeline's type inference, not a mistake in the runtime signature. The fix is upstream: fix the IR type (e.g. `instance_find` returning `number` instead of `GMLObject|null`), not the callee. Similarly, TS2554 (wrong arg count) is fixed by a targeted IR transform pass that widens per-function arity based on call-site analysis — never by adding `...args: any[]` or by making every function globally variadic. Transform passes write sigs; emitters read them.
