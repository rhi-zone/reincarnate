...
**`Type::Unknown` is concrete — a terminal type, not a free variable.** In the type lattice Unknown is concrete by design: `is_concrete(Type::Unknown)` returns `true` because inference is exhausted at this site and there is no arena variable to bind (see `constraint_collect.rs`: `// decided: inference exhausted, not a free variable`). Unknown is NOT an `InferVar`. Do not treat it as a solvable variable, do not narrow other types against it, and do not report `is_concrete(Unknown) == true` — or the `saw_unknown` evidence-completeness gate that depends on it — as a bug; that is correct behavior. A call site supplying an `Unknown` argument genuinely passes an unknown-typed value, so it correctly blocks narrowing the callee param on incomplete evidence. Separately, and without contradiction, emit quality is a different axis: emitting `unknown` where the source value has a determinate type that better inference would recover is a defect — improve the inference; where a value's type is genuinely undetermined, leave Unknown in the IR, emit the inference-failure diagnostic, and file a TODO. Suppressing Unknown at emit, or propagating types by guessing from downstream uses, are both monkeypatches. Unknown is a legitimate terminal type in the solver; depending on it where a concrete source type exists is the failure, not the type itself.
