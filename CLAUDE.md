...
- **Take the correct approach, not the simple or complex one.** Never optimize for ease of implementation or minimize scope to dodge difficulty. Never add complexity for its own sake either. Ask: what is the *right* design? Then do that. "Simpler" and "more complex" are not axes of quality — correctness is.
