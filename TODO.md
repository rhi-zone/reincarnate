...
- [ ] **Phase 9 — Runtime as IR.** GML stdlib expressed as IR *functions with bodies*; `RuntimeRegistry` maps stdlib names to `FuncId`s. Frontends emit the runtime the same way they emit game code — same `Function` struct, same pipeline. Backends emit runtime functions like any other function; no per-backend runtime package. Intrinsics (single primitive op in every target: `abs`, `floor`, `sqrt`) get `Function::intrinsic: Option<IntrinsicKind>` and a small backend lookup table instead of a body. Deletes `runtime/gamemaker/runtime.ts` as handwritten source (it becomes generated output). Gate: same output, M+N architecture (not M×N).
  ...
  > **Monomorphic overload selection**: stdlib functions with GML-polymorphic signatures (e.g. `int :: Real → Int | String → Int`) must be expressed as separate monomorphic IR functions (`int_real`, `int_string`, …) with a post-HM lowering pass that rewrites call sites to the concrete overload wherever the argument type is resolved. The `unknown`-param fallback overload is only emitted at genuinely unresolved sites — it is an inference gap made visible, not a legitimate default. This is required for all backends: TypeScript avoids IC megamorphism (V8 deopt); Rust avoids a `GmlValue` enum dispatch (tag check + branch on every call, plus oversized stack slots) where a monomorphic call compiles to a single instruction. The `unknown`-param runtime functions in the current handwritten `runtime.ts` are load-bearing tech debt that Phase 9 deletes.
  > **Inline hints**: `Function::inline_hint: Option<InlineHint>` (Always / Never / Default) set by the runtime definition. Backends emit the appropriate annotation (`#[inline(always)]` for Rust; actual IR inlining for TS). Manifest can override per-function for debug vs release builds.
  > **Core builtin registry**: typed arithmetic (`add_f64`, `add_i32`, …) currently registered by the GML frontend — a Law 2 violation. These belong in core, auto-registered on every `Module`, because they are universal operations whose semantics are identical across all source languages. The same applies to trig (`sin`, `cos`, `sqrt`, `log`, `exp`) and unambiguous string primitives (`string_byte_length`, `string_codepoint_length`, `string_concat`). The completeness criterion: IR builtins must be expressive enough to write every engine's runtime bodies without circularity. Where an operation has ambiguous semantics across source languages (e.g. `string_length` = bytes in GML, codepoints elsewhere), core provides the unambiguous primitives and each engine stdlib is a one-line wrapper that selects the right one. Engine-specific ops (GML `draw_text`, SugarCube `Engine.play`) remain in engine stdlib only.
