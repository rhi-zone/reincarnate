...
`Type::Unknown` is a post-inference signal that the solver could not determine the type. It represents an inference gap and is emitted as `unknown` in TypeScript. Every `Unknown` in output IR is a defect — inference was not good enough. Any pre-inference occurrence (e.g. in `Constant::Null`) is also a defect — it should be `Type::Var`.
