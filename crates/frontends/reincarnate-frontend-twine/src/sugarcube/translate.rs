...
impl TranslateCtx {
    ...
    fn lower_oxc_expr(&mut self, expr: &js::Expression<'_>, pp: &Preprocessed) -> ValueId {
        ...
        match expr {
            ...
            js::Expression::CallExpression(call) => {
                // Fast path: bare identifier callees for known SugarCube stdlib functions.
                // Emit a typed SystemCall directly instead of Engine.resolve("name")(...).
                if let js::Expression::Identifier(ident) = &call.callee {
                    let name = ident.name.as_str();
                    if !name.starts_with('$') && !name.starts_with('_') {
                        if let Some(result) = self.lower_stdlib_call(name, &call.arguments, pp) {
                            return result;
                        }
                    }
                }
                // General case: lower callee, then call_indirect.
    ...
    /// Try to lower a bare-identifier call as a typed SugarCube stdlib SystemCall.
    ///
    /// Returns `Some(ValueId)` when `name` is a known stdlib function, mapping it
    /// to a direct `SystemCall` with a concrete return type.  Returns `None` for
    /// unknown names, allowing the caller to fall through to the
    /// `Engine.resolve("name")(...)`/`call_indirect` general path.
    ///
    /// Each entry was verified against the runtime TypeScript sources:
    /// - `SugarCube.Navigation` methods: `visited`, `lastVisited`, `tags`,
    ///   `passage`, `turns`, `previous`, `visitedTags`
    /// - `SugarCube.Engine` methods: `random`, `either`, `isNaN`, `isFinite`,
    ///   `parseInt`, `parseFloat`
    fn lower_stdlib_call(
        &mut self,
        name: &str,
        arguments: &oxc_allocator::Vec<'_, js::Argument<'_>>,
        pp: &Preprocessed,
    ) -> Option<ValueId> {
        ...
        match name {
            ...
            // visited(...passages: string[]) => number
            // Count the minimum number of times any of the named passages has been visited.
            // When called with no arguments, returns the visit count for the current passage.
            "visited" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Navigation", "visited", &args, Type::Int(64)),
                )
            }

            // lastVisited(...passages: string[]) => number
            // Returns the number of turns since a passage was last visited (-1 if never).
            "lastVisited" => Some(self.fb.system_call(
                "SugarCube.Navigation",
                "lastVisited",
                &args,
                Type::Int(64),
            ...
            // tags(...passages: string[]) => string[]
            // Returns all tags for the named passages (or the current passage if none given).
            "tags" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Navigation", "tags", &args, Type::Dynamic),
                )
            }

            // passage() => string
            // Returns the current passage name.
            "passage" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Navigation", "passage", &args, Type::String),
                )
            }

            // turns() => number
            // Returns the number of moments in the history (number of passages visited so far).
            "turns" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Navigation", "turns", &args, Type::Int(64)),
                )
            }

            // previous() => string
            // Returns the title of the most recently visited passage before the current one.
            "previous" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Navigation", "previous", &args, Type::String),
                )
            }

            // visitedTags(...tags: string[]) => number
            // Returns the number of passages that have all of the given tags.
            "visitedTags" => Some(self.fb.system_call(
                "SugarCube.Navigation",
                "visitedTags",
                &args,
                Type::Int(64),
            ...
            // Returns a random integer in [min, max] (or [0, min] when called with one arg).
            "random" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Engine", "random", &args, Type::Int(64)),
                )
            }

            // either<T>(...args: T[]) => T
            // Returns a random element from its arguments.
            "either" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Engine", "either", &args, Type::Dynamic),
                )
            }
            ...
            "isNaN" => Some(
                self.fb
                    .system_call("SugarCube.Engine", "isNaN", &args, Type::Bool),
            ),
            ...
            "isFinite" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Engine", "isFinite", &args, Type::Bool),
                )
            }
            ...
            "parseInt" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Engine", "parseInt", &args, Type::Int(64)),
                )
            }
            ...
            "parseFloat" => {
                Some(
                    self.fb
                        .system_call("SugarCube.Engine", "parseFloat", &args, Type::Float(64)),
                )
            }
