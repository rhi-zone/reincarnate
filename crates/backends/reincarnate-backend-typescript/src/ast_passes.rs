...
// ---------------------------------------------------------------------------
// fold_array_init_push
// ---------------------------------------------------------------------------

/// Fold `const x = [items...]; x.push(a, b);` into `const x = [items..., a, b];`.
///
/// When the translator emits push-based content, simple passages produce:
///   const __out = [];
///   __out.push(a, b, c);
///   return __out;
/// This pass folds the push args into the ArrayInit so later passes can
/// inline the array directly into the return.
pub fn fold_array_init_push(body: &mut Vec<JsStmt>) {
    // Recurse into nested bodies first.
    for stmt in body.iter_mut() {
        fold_array_init_push_in_stmt(stmt);
    }

    // Fold at this level.
    let mut i = 0;
    while i + 1 < body.len() {
        // Check: body[i] is VarDecl with ArrayInit, body[i+1] is push on same var.
        let var_name = match &body[i] {
            JsStmt::VarDecl {
                name,
                init: Some(JsExpr::ArrayInit(_)),
                ..
            } => name.clone(),
            _ => {
                i += 1;
        ...
        if let Some(push_args) = extract_push_on_var(&body[i + 1], &var_name) {
            let push_args = push_args.clone();
            // Move push args into the ArrayInit.
            if let JsStmt::VarDecl {
                init: Some(JsExpr::ArrayInit(elems)),
                ..
            } = &mut body[i]
            {
                elems.extend(push_args);
            }
            body.remove(i + 1);
            // Don't increment — there may be another push to fold.
        } else {
            i += 1;
        }
    }
...
fn fold_array_init_push_in_stmt(stmt: &mut JsStmt) {
    match stmt {
        JsStmt::If {
            then_body,
            else_body,
            ...
            fold_array_init_push(then_body);
            fold_array_init_push(else_body);
        }
        JsStmt::While { body, .. }
        | JsStmt::Loop { body }
        | JsStmt::ForOf { body, .. } => {
            fold_array_init_push(body);
        }
        JsStmt::For {
            init,
            body,
            ...
            fold_array_init_push(init);
            fold_array_init_push(body);
            fold_array_init_push(update);
        }
        JsStmt::Switch {
            cases,
            default_body,
            ...
            for (_, case_body) in cases {
                fold_array_init_push(case_body);
            }
            fold_array_init_push(default_body);
        }
        JsStmt::Dispatch { blocks, .. } => {
            for (_, block_body) in blocks {
                fold_array_init_push(block_body);
            }
        }
        _ => {}
    }
...
/// Merge consecutive `x.push(a); x.push(b, c);` into `x.push(a, b, c);`.
pub fn merge_adjacent_push(body: &mut Vec<JsStmt>) {
    // Recurse into nested bodies first.
    for stmt in body.iter_mut() {
        merge_adjacent_push_in_stmt(stmt);
    }

    // Merge at this level.
    let mut i = 0;
    while i + 1 < body.len() {
        let first_var = match extract_push_var(&body[i]) {
        ...
        if extract_push_var(&body[i + 1]).is_some_and(|name| name == first_var) {
            // Move args from body[i+1] into body[i].
            let extra_args = extract_push_args_owned(&mut body[i + 1]);
            append_push_args(&mut body[i], extra_args);
            body.remove(i + 1);
            // Don't increment — there may be more to merge.
        } else {
            i += 1;
        }
    }
...
fn merge_adjacent_push_in_stmt(stmt: &mut JsStmt) {
    match stmt {
        JsStmt::If {
            then_body,
            else_body,
            ...
            merge_adjacent_push(then_body);
            merge_adjacent_push(else_body);
        }
        JsStmt::While { body, .. }
        | JsStmt::Loop { body }
        | JsStmt::ForOf { body, .. } => {
            merge_adjacent_push(body);
        }
        JsStmt::For {
            init,
            body,
            ...
            merge_adjacent_push(init);
            merge_adjacent_push(body);
            merge_adjacent_push(update);
        }
        JsStmt::Switch {
            cases,
            default_body,
            ...
            for (_, case_body) in cases {
                merge_adjacent_push(case_body);
            }
            merge_adjacent_push(default_body);
        }
        JsStmt::Dispatch { blocks, .. } => {
            for (_, block_body) in blocks {
                merge_adjacent_push(block_body);
            }
        }
        _ => {}
    }
...
/// Extract args from `Expr(Call { callee: Field { object: Var(name), field: "push" }, args })`
/// if the var matches.
fn extract_push_on_var<'a>(stmt: &'a JsStmt, var_name: &str) -> Option<&'a Vec<JsExpr>> {
    if let JsStmt::Expr(JsExpr::Call { callee, args }) = stmt {
        if let JsExpr::Field { object, field } = callee.as_ref() {
            if field == "push" {
                if let JsExpr::Var(name) = object.as_ref() {
                    if name == var_name {
                        return Some(args);
                    }
                }
...
/// Extract the variable name from a push statement.
fn extract_push_var(stmt: &JsStmt) -> Option<&str> {
    if let JsStmt::Expr(JsExpr::Call { callee, .. }) = stmt {
        if let JsExpr::Field { object, field } = callee.as_ref() {
            if field == "push" {
                if let JsExpr::Var(name) = object.as_ref() {
                    return Some(name);
...
/// Take ownership of args from a push statement.
fn extract_push_args_owned(stmt: &mut JsStmt) -> Vec<JsExpr> {
    if let JsStmt::Expr(JsExpr::Call { args, .. }) = stmt {
        std::mem::take(args)
    } else {
        vec![]
    }
...
/// Append args to an existing push statement.
fn append_push_args(stmt: &mut JsStmt, extra: Vec<JsExpr>) {
    if let JsStmt::Expr(JsExpr::Call { args, .. }) = stmt {
        args.extend(extra);
    }
}
...
mod tests {
    ...
    fn push_call(var_name: &str, args: Vec<JsExpr>) -> JsStmt {
        JsStmt::Expr(JsExpr::Call {
            callee: Box::new(JsExpr::Field {
                object: Box::new(JsExpr::Var(var_name.into())),
                field: "push".into(),
            }),
            args,
        })
    ...
    fn is_str_lit(expr: &JsExpr, expected: &str) -> bool {
        matches!(expr, JsExpr::Literal(Constant::String(s)) if s == expected)
    }

    fn is_spread_var(expr: &JsExpr, expected: &str) -> bool {
        matches!(expr, JsExpr::Spread(inner) if matches!(inner.as_ref(), JsExpr::Var(n) if n == expected))
    }

    #[test]
    fn test_fold_array_init_push_basic() {
        // const x = [a]; x.push(b, c); → const x = [a, b, c];
        let mut body = vec![
            JsStmt::VarDecl {
                name: "__out".into(),
                ty: None,
                init: Some(JsExpr::ArrayInit(vec![str_lit("a")])),
                mutable: false,
            },
            push_call("__out", vec![str_lit("b"), str_lit("c")]),
        ];

        fold_array_init_push(&mut body);
        ...
        match &body[0] {
            JsStmt::VarDecl {
                ...
                assert!(is_str_lit(&elems[0], "a"));
                assert!(is_str_lit(&elems[1], "b"));
                assert!(is_str_lit(&elems[2], "c"));
            }
            other => panic!("Expected VarDecl with ArrayInit, got: {other:?}"),
        }
    }

    #[test]
    fn test_fold_array_init_push_consecutive() {
        // const x = []; x.push(a); x.push(b); → const x = [a, b];
        let mut body = vec![
            JsStmt::VarDecl {
                name: "__out".into(),
                ty: None,
                init: Some(JsExpr::ArrayInit(vec![])),
                mutable: false,
            },
            push_call("__out", vec![str_lit("a")]),
            push_call("__out", vec![str_lit("b")]),
        ];
    ...
    #[test]
    fn test_fold_array_init_push_stops_at_non_push() {
        // const x = []; x.push(a); if (...) { ... } x.push(b);
        // Only folds first push, second stays.
        let mut body = vec![
            JsStmt::VarDecl {
                name: "__out".into(),
                ty: None,
        ...
        match &body[0] {
            ...
            other => panic!("Expected VarDecl with folded push, got: {other:?}"),
        }
        assert!(matches!(&body[1], JsStmt::If { .. }));
    }

    #[test]
    fn test_fold_array_init_push_spread() {
        // const x = []; x.push(...arr); → const x = [...arr];
        let mut body = vec![
            JsStmt::VarDecl {
                name: "__out".into(),
                ty: None,
            ...
            push_call("__out", vec![JsExpr::Spread(Box::new(var("arr")))]),
        ];
        ...
        match &body[0] {
            JsStmt::VarDecl {
                ...
                assert_eq!(elems.len(), 1);
                assert!(is_spread_var(&elems[0], "arr"));
            }
            other => panic!("Expected ArrayInit with spread, got: {other:?}"),
        }
    }

    #[test]
    fn test_merge_adjacent_push_basic() {
        // x.push(a); x.push(b, c); → x.push(a, b, c);
        let mut body = vec![
            push_call("x", vec![str_lit("a")]),
            push_call("x", vec![str_lit("b"), str_lit("c")]),
        ];

        merge_adjacent_push(&mut body);

        assert_eq!(body.len(), 1);
        match &body[0] {
            JsStmt::Expr(JsExpr::Call { args, .. }) => {
                assert_eq!(args.len(), 3);
                assert!(is_str_lit(&args[0], "a"));
                assert!(is_str_lit(&args[1], "b"));
                assert!(is_str_lit(&args[2], "c"));
            }
            other => panic!("Expected merged push call, got: {other:?}"),
        }
    }

    #[test]
    fn test_merge_adjacent_push_different_vars() {
        // x.push(a); y.push(b); — different receivers, no merge.
        let mut body = vec![
            push_call("x", vec![str_lit("a")]),
            push_call("y", vec![str_lit("b")]),
        ];
    ...
    #[test]
    fn test_merge_adjacent_push_three() {
        // x.push(a); x.push(b); x.push(c); → x.push(a, b, c);
        let mut body = vec![
            push_call("x", vec![str_lit("a")]),
            push_call("x", vec![str_lit("b")]),
            push_call("x", vec![str_lit("c")]),
        ];
