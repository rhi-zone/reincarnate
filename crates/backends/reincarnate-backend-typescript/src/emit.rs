...
/// out known false positives (private/protected namespace accesses, Flash
/// authoring/runtime library types).
fn warn_unmapped_reference(name: &str) {
    ...
    // Flash runtime stdlib — actionable: add to runtime. Warn specifically.
    if let Some(pkg) = ns.strip_prefix("flash.") {
        eprintln!("warning: flash package '{pkg}' not in runtime stdlib (referenced: {name})");
        return;
    }
    // Flash-specific namespaces (fl.*, mx.*) — silent, not actionable.
...
fn emit_class(
    ...
    if engine == EngineKind::Flash {
        out.push_str(&crate::rewrites::flash::emit_flash_class_header(&qualified));
    }
    ...
    // Flash: emit forwarding setters for getters that have no corresponding
    // setter, to prevent TS2540 ("Cannot assign to '…' because it is
    // read-only property").
    if engine == EngineKind::Flash {
        // Build the set of setter bare names already present in this class.
        let setter_names: HashSet<String> = group
            .methods
            .iter()
            .filter(|&&fid| module.functions[fid].method_kind == MethodKind::Setter)
            .map(|&fid| {
                let short = module.functions[fid]
                    .name
                    .rsplit("::")
                    .next()
                    .unwrap_or(&module.functions[fid].name);
                short.strip_prefix("set_").unwrap_or(short).to_string()
            })
            .collect();
        for &fid in &group.methods {
            let func = &module.functions[fid];
            if func.method_kind != MethodKind::Getter {
                continue;
            }
            let short = func.name.rsplit("::").next().unwrap_or(&func.name);
            let prop = short.strip_prefix("get_").unwrap_or(short);
            if !setter_names.contains(prop) {
                let ret_ty = crate::types::ts_type(&func.sig.return_ty);
                // Needs `override` if the property is defined in a parent class.
                let is_setter_override = parent_method_names.contains(prop);
                out.push('\n');
                out.push_str(&crate::rewrites::flash::emit_forwarding_setter(
                    &class_name,
                    prop,
                    &ret_ty,
                    is_setter_override,
    ...
    if engine == EngineKind::Flash {
        crate::rewrites::flash::emit_class_registration(group, module, &class_name, out);
    }
    Ok(())
}
...
fn emit_class_method(
    ...
    let flash_ctor_extra_param: Option<String> =
        if engine == EngineKind::Flash && func.method_kind == MethodKind::Constructor {
            crate::rewrites::flash::flash_ctor_shims_param(suppress_super)
        } else {
            None
        };
