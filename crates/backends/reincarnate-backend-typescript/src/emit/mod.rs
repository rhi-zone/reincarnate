...
fn emit_class_file(
    ...
    let all_systems = collect_system_names_from_funcs(class_funcs(), None);
    ...
    let func_names_class: HashMap<FuncId, String> = module
        ...
        .collect();
    let calls = collect_call_names_from_funcs(class_funcs(), engine, None, &func_names_class);
...
fn emit_runtime_functions_file(
    ...
    let calls = collect_call_names_from_funcs(rt_fn_iter(), engine, None, &func_names_rt);
...
fn emit_free_functions_file(
    ...
    let name_map: HashMap<String, String> = module
        ...
        .map(|(raw, ts)| (raw.clone(), ts.clone()))
        .collect();
    let all_free_systems = collect_system_names_from_funcs(free_fn_iter(), None);
    ...
    let func_names_free: HashMap<FuncId, String> = module
        ...
        .map(|fid| (fid, module.func_name(fid).to_string()))
        .collect();
    let calls = collect_call_names_from_funcs(free_fn_iter(), engine, None, &func_names_free);
...
pub fn emit_module_to_dir(
    ...
    let func_names_global: HashMap<FuncId, String> = module
        ...
        .collect();
    let imported_names =
        collect_call_names_from_funcs(module.functions.values(), engine, None, &func_names_global);
